...
SECURITY_STATUS SEC_ENTRY sspi_acquire_default_credentials_handle(CredHandle *cred_handle)
{
	...
	return call_sspi_acquire_credentials_handle(NULL, SSPI_PACKAGE_NAME, SECPKG_CRED_OUTBOUND, NULL, NULL, NULL, NULL, cred_handle, &ignored);
}
...
SECURITY_STATUS SEC_ENTRY sspi_get_cred_name(CredHandle *cred_handle, char **out_name)
{
	...
	SECURITY_STATUS status = call_sspi_query_credentials_attributes(cred_handle, SECPKG_CRED_ATTR_NAMES, &names);
	...
	*out_name = malloc(len);
	memcpy(*out_name, names.sUserName, len);

	return call_sspi_free_context_buffer(names.sUserName);
}

SECURITY_STATUS SEC_ENTRY sspi_free_credentials_handle(CredHandle *cred_handle)
{
	return call_sspi_free_credentials_handle(cred_handle);
}

SECURITY_STATUS SEC_ENTRY sspi_initialize_security_context(CredHandle *cred_handle, int has_context, CtxtHandle *context, PVOID buffer, ULONG buffer_length, PVOID *out_buffer, ULONG *out_buffer_length,  char *spn)
{
	SecBufferDesc inbuf;
	SecBuffer in_bufs[1];
...
{
	return call_sspi_delete_security_context(context);
}
